Feature: PutObject
  ...
  Scenario: PutObject: overwrites regular file
    Given `rms.jpg` was uploaded to `data` bucket as `a/b/file`
    When `ignucius.jpg` is uploaded to `data` bucket as `a/b/file`
    Then `ignucius.jpg` is stored as `data/a/b/file`

  Scenario: PutObject: overwrites symlink
    Given `rms.jpg` was uploaded to `data` bucket as `dir/file1`
    And `ignucius.jpg` was uploaded to `data` bucket as `dir/file2`
    And `link` symlink was created on `links` bucket pointing to `data/dir/file1`
