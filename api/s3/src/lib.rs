...
pub async fn run_http_server<S, A>(storage: S, addr: A) -> Result<(), RunHttpServerError>
where
    A: ToSocketAddrs,
    S3<S>: S3Storage + Send + Sync + 'static,
{
    ...
    let make_service = make_service_fn(move |_| future::ready(Ok::<_, Infallible>(service.clone())));
    ...
    E2: fmt::Display,
{
    ...
    async fn put_object(
        &self, input: dto::PutObjectRequest,
    ) -> S3StorageResult<dto::PutObjectOutput, dto::PutObjectError> {
        let path = parse_s3_path(input.bucket, input.key)?;

        if let Some(original) = input.metadata.and_then(|mut meta| meta.remove(SYMLINK_META_KEY)) {
            let op = Symlink { original: parse_relative_path(SYMLINK_META_KEY, original)?, link: path };

            self.0.exec(op).await.map_err(|e| internal_error("Symlink operation failed", e))?;
        } else {
            let op = CreateFile { path, bytes: input.body.unwrap_or_else(|| vec![].into()) };

            self.0.exec(op).await.map_err(|e| internal_error("CreateFile operation failed", e))?;
        };
...
fn parse_s3_path(bucket: String, key: String) -> Result<RelativePath, S3Error> {
    Ok(parse_relative_path("bucket", bucket)?.join(parse_relative_path("key", key)?))
}

/// Parses a string into [`RelativePath`].
fn parse_relative_path(attr: &str, s: String) -> Result<RelativePath, S3Error> {
    s.try_into().map_err(|e| S3Error::new(S3ErrorCode::InvalidArgument, format!("Invalid {attr}: {e}")))
}
